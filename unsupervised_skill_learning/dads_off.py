...
flags.DEFINE_integer('run_eval_loop', 0, 'Only run eval loop function')
...
flags.DEFINE_float('horizontal_wrist_constraint', 1.0,
...
flags.DEFINE_integer('num_candidate_sequences', 50,
...
def relabel_skill(trajectory_sample,
    ...
    if relabel_type is None or ('importance_sampling' in relabel_type and
        ...
        # print("no relabeling for trajectory_sample !!!")
...
def eval_loop(eval_dir,
    ...
    min_len = min(len(traj_std) for traj_std in Traj_std)
    Traj_std = [traj_std[:min_len] for traj_std in Traj_std]
    ...
    if plot_name is not None:
        ...
        if plot_std:
            ...
            plt.fill_between(np.arange(len(error_bar)), Traj_std_mean - error_bar, Traj_std_mean + error_bar, alpha=0.2, facecolor='blue')
            # to save images while writing to CNS
            buf = io.BytesIO()
    ...
    with tf.compat.v2.summary.record_if(True):
        ...
        with tf.compat.v1.Session().as_default() as sess:
            ...
            if FLAGS.run_train:
                ...
                def _filter_trajectories(trajectory, filter_more=False):
                    ...
                    if filter_more:
                        valid_indices1 = (trajectory.step_type[:, 1:] != 0)
                        valid_indices1 = np.array([np.all(valid_indices1[i, :]) for i in range(valid_indices1.shape[0])])
                        valid_indices = np.logical_and(valid_indices, valid_indices1)
                ...
                while iter_count < FLAGS.num_epochs:
                    ...
                    for _ in range(1 if FLAGS.clear_buffer_every_iter else FLAGS
                        ...
                        trajectory_sample = _filter_trajectories(trajectory_sample, filter_more=FLAGS.learn_slow_feature)
